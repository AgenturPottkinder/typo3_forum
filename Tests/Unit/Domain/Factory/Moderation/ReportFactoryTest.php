...
class ReportFactoryTest extends \TYPO3\CMS\Extbase\Tests\Unit\BaseTestCase {
	...
	public function setUp() {
		...
		$this->workflowStatusRepositoryMock = $this->getMock('Mittwald\\MmForum\\Domain\\Repository\\Moderation\\ReportWorkflowStatusRepository');
		$this->workflowStatusRepositoryMock->expects($this->any())->method('findInitial')
			->will($this->returnValue($this->initialStatus = new \Mittwald\MmForum\Domain\Model\Moderation\ReportWorkflowStatus('Open', TRUE, FALSE)));

		$this->fixture = new ReportFactory($this->workflowStatusRepositoryMock);
	...
	public function createReportCreatesNewReportFromComment() {
		$user = new \Mittwald\MmForum\Domain\Model\User\FrontendUser('martin', 'secret');
		...
		$comment = new \Mittwald\MmForum\Domain\Model\Moderation\ReportComment($user, 'Content');
		$report  = $this->fixture->createReport($comment, $post);

		$this->assertInstanceOf('Mittwald\\MmForum\\Domain\\Model\\Moderation\\Report', $report);

...
class Tx_MmForum_ViewHelpers_Post_HelpfulButtonViewHelper extends \TYPO3\CMS\Fluid\ViewHelpers\CObjectViewHelper {
	...
	public function render(Tx_MmForum_Domain_Model_Forum_Post $post, $countTarget = NULL, $countUserTarget = NULL) {
		...
		$btn = '<div class="' . $class . ' tx-mmforum-helpfull-btn" data-countusertarget="'.$countUserTarget.'" data-counttarget="'.$countTarget.'" data-post="'.$post->getUid().'" data-pageuid="'.$this->settings['pids']['Forum'].'" data-eid="'.$this->settings['forum']['post']['helpfulBtn']['eID'].'"></div>';
		return $btn;
	}
}

 ...
 * ACL entries can be assigned to any \Mittwald\MmForum\Domain\Model\Forum\Forum object and
 * are inherited down the forum tree unto each single post.
 *
 * Every object that implements the \Mittwald\MmForum\Domain\Model\AccessibleInterface
...
class Access extends \TYPO3\CMS\Extbase\DomainObject\AbstractValueObject {
	...
	public function __construct($operation = NULL, $level = NULL,
	                            \Mittwald\MmForum\Domain\Model\User\FrontendUserGroup $group = NULL) {
	 ...
	 * @return \Mittwald\MmForum\Domain\Model\User\FrontendUserGroup group The group
	 */
	 ...
	 */
	public function matches(\Mittwald\MmForum\Domain\Model\User\FrontendUser $user = NULL) {
		...
		if ($this->loginLevel === self::LOGIN_LEVEL_SPECIFIC) {
			foreach($user->getUsergroup() as $group) {
				/** @var $group \Mittwald\MmForum\Domain\Model\User\FrontendUserGroup */
	 ...
	 * Sets the group.
	 *
	 * @param \Mittwald\MmForum\Domain\Model\User\FrontendUserGroup $group The group
	 * @return void
	 */
	public function setAffectedGroup(\Mittwald\MmForum\Domain\Model\User\FrontendUserGroup $group) {
		$this->affectedGroup = $group;
	}
